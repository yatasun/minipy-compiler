...
class Compiler:
    ...
    def rco_exp(self, e: expr, need_atomic: bool) -> Tuple[expr, Temporaries]:
        ...
        match e:
            ...
            case IfExp(exp1, exp2, exp3):
                (new_exp1, temps1) = self.rco_exp(exp1, False)
                (new_exp2, temps2) = self.rco_exp(exp2, False)
                (new_exp3, temps3) = self.rco_exp(exp3, False)

                then_branch = make_begin(temps2, new_exp2)
                else_branch = make_begin(temps3, new_exp3)
                result_expr, result_temps = (
                    IfExp(new_exp1, then_branch, else_branch),
                    temps1
                )
    ...
    def prelude_and_conclusion(self, p: X86Program) -> X86Program:
        new_body = (
            [
                ...
                Instr("subq", [Immediate(align(self.spilled_size, 16)), Reg("rsp")]),
            ]
            + p.body  # type: ignore
            + [
                Instr("addq", [Immediate(align(self.spilled_size, 16)), Reg("rsp")]),
                Instr("popq", [Reg("rbp")]),
                Instr("retq", []),
            ]
        )
